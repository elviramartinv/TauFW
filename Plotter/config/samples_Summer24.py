...
def getsampleset(channel,era,**kwargs):
  ...
  if '2022_preEE' in era or '2022_postEE' in era or '2023'in era or '2024' in era: # so far same samples and cross sections are used for preEE and postEE, if event numbers are set elsewhere then we don't need to add seperate numbers for both eras
    ...
    if '2024' in era:
      expsamples = [ # table of MC samples to be converted to Sample objects
        ...
        ( 'WJ', "WtoTauNu-2Jets",            "W + jets",           22666.*kfactor_wj ), # LO times kfactor
        # ( 'WJ', "WtoTauNu-2Jets_0J",            "W + 0J",           18586.*kfactor_wj ), # LO times kfactor
        ( 'WJ', "WtoMuNu-2Jets",            "W + jets",           22666.*kfactor_wj ), # LO times kfactor
        # ( 'WJ', "WtoMuNu-2Jets_0J",            "W + 0J",           18586.*kfactor_wj ), # LO times kfactor
        ( 'WJ', "WtoENu-2Jets",            "W + jets",           22666.*kfactor_wj ), # LO times kfactor
  ...
  # if '2024' in era:
  #     sampleset.stitch("W*Nu*Jets*",   incl='WtoTauNu-2Jets',  name='WJ', cme=cme) # W + jets
  ...
  sampleset.join('W*J*', name='WJ' ) # W + jets
  if 'VV' in join:
